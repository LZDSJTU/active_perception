...
![一次闭环，三种交互，一次视觉判断](doc/assets/poster_closed_loop.png)

每个候选严格遵循 `push → press → lift_box` 的属性依赖顺序：系统先通过推动验证可移动性，再通过垂直点击验证按钮响应，最后抓取并移开无底盒，由本地 Qwen 判断原位置是否出现绿色小球。不满足前置属性的盒子会被立即排除；三项属性全部成立时，机器人选择该盒并停止。
